...
def handle_vincinv():
    ...
    ell_dist, azimuth1to2_dd, azimuth2to1_dd = vincinv(
        lat1_dd, lon1_dd, lat2_dd, lon2_dd
    )
    ...
    return jsonify(
        {"ell_dist": ell_dist, "azimuth1to2": azimuth1to2, "azimuth2to1": azimuth2to1}
    ), 200
...
def handle_vincdir():
    ...
    lat2_dd, lon2_dd, azimuth2to1_dd = vincdir(
        lat1_dd, lon1_dd, azimuth1to2_dd, ell_dist
    )

    angle_type = dd_to_angle_type[to_angle_type]
    lat2, lon2, azimuth2to1 = (
        angle_type(lat2_dd),
        angle_type(lon2_dd),
        angle_type(azimuth2to1_dd),
    )

    return jsonify({"lat2": lat2, "lon2": lon2, "azimuth2to1": azimuth2to1}), 200
