...
class Transformation(object):
    def __init__(
        ...
        :param from_datum: Datum transforming from
        :param to_datum: Datum transforming to
        :param ref_epoch: Ref. epoch
        :type ref_epoch: datetime.date
        :param tx: Translation in X (m)
        :param ty: Translation in Y (m)
        :param tz: Translation in Z (m)
        :param sc: Scale factor (ppm)
        :param rx: Rotation about X (arcsec)
        :param ry: Rotation about Y (arcsec)
        :param rz: Rotation about Z (arcsec)
        :param d_tx: Translation rate of change in X (m/yr)
        :param d_ty: Translation rate of change in Y (m/yr)
        :param d_tz: Translation rate of change in Z (m/yr)
        :param d_sc: Scale factor rate of change (ppm/yr)
        :param d_rx: Rotate rate of change about X (arcsec/yr)
        :param d_ry: Rotate rate of change about Y (arcsec/yr)
        :param d_rz: Rotate rate of change about Z (arcsec/yr)
        :param tf_sd: TransformationSD object
        :type tf_sd: TransformationSD
        """
    ...
    def __add__(self, other):
        ...
        if type(other) == date:
            ...
            if type(self.tf_sd) == TransformationSD:
                self.tf_sd.sd_tx = (
                    self.tf_sd.sd_tx**2 + (self.tf_sd.sd_d_tx * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_ty = (
                    self.tf_sd.sd_ty**2 + (self.tf_sd.sd_d_ty * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_tz = (
                    self.tf_sd.sd_tz**2 + (self.tf_sd.sd_d_tz * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_sc = (
                    self.tf_sd.sd_sc**2 + (self.tf_sd.sd_d_sc * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_rx = (
                    self.tf_sd.sd_rx**2 + (self.tf_sd.sd_d_rx * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_ry = (
                    self.tf_sd.sd_ry**2 + (self.tf_sd.sd_d_ry * timediff) ** 2
                ) ** 0.5
                self.tf_sd.sd_rz = (
                    self.tf_sd.sd_rz**2 + (self.tf_sd.sd_d_rz * timediff) ** 2
                ) ** 0.5
...
class TransformationSD(object):
    def __init__(
        ...
        :param sd_ty: one-sigma uncertainty of ty (m)
        :param sd_tz: one-sigma uncertainty of tz (m)
        :param sd_sc: one-sigma uncertainty of sc (ppm)
        :param sd_rx: one-sigma uncertainty of rx (arcsec/yr)
        :param sd_ry: one-sigma uncertainty of ry (arcsec/yr)
        :param sd_rz: one-sigma uncertainty of rz (arcsec/yr)
        :param sd_d_tx: one-sigma uncertainty of d_tx (m/yr)
        :param sd_d_ty: one-sigma uncertainty of d_ty (m/yr)
        :param sd_d_tz: one-sigma uncertainty of d_tz (m/yr)
        :param sd_d_sc: one-sigma uncertainty of d_sc (ppm/yr)
        :param sd_d_rx: one-sigma uncertainty of d_rx (arcsec/yr)
...
# Ref: http://itrf.ign.fr/en/solutions/transformations
