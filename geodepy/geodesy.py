...
def vincdir(lat1, lon1, azimuth1to2, ell_dist, ellipsoid=grs80):
    ...
    lat2 = atan2(
        ...
        * sqrt(
            sin(alpha) ** 2
            + (sin(u1) * sin(sigma) - cos(u1) * cos(sigma) * cos(azimuth1to2)) ** 2
        ),
    )
    lat2 = degrees(lat2)
    ...
    azimuth2to1 = (
        degrees(
            atan2(
                sin(alpha),
                -sin(u1) * sin(sigma) + cos(u1) * cos(sigma) * cos(azimuth1to2),
            )
        )
        + 180
    )
...
def vincinv(lat1, lon1, lat2, lon2, ellipsoid=grs80):
    ...
    for i in range(1000):
        ...
        new_lon = omega + (1 - c) * ellipsoid.f * sin(alpha) * (
            ...
            + c
            * sin(sigma)
            * (cos_two_sigma_m + c * cos(sigma) * (-1 + 2 * (cos_two_sigma_m**2)))
        )
    ...
    azimuth1to2 = degrees(
        atan2((cos(u2) * sin(lon)), (cos(u1) * sin(u2) - sin(u1) * cos(u2) * cos(lon)))
    )
    ...
    azimuth2to1 = (
        degrees(
            atan2(
                cos(u1) * sin(lon), (-sin(u1) * cos(u2) + cos(u1) * sin(u2) * cos(lon))
            )
        )
        + 180
    )
...
def vincdir_utm(
    zone1, east1, north1, grid1to2, grid_dist, hemisphere="south", ellipsoid=grs80
):
    """
    ...
    lat1, lon1, psf1, gridconv1 = grid2geo(zone1, east1, north1, hemisphere, ellipsoid)

    # Convert Grid Bearing to Geodetic Azimuth
    az1to2 = grid1to2 - gridconv1

    # Estimate Line Scale Factor (LSF)
    zone2, east2, north2 = (zone1, *radiations(east1, north1, grid1to2, grid_dist))
    ...
    while lsf_diff > 1e-9:
        lsf_previous = lsf
        lat2, lon2, az2to1 = vincdir(lat1, lon1, az1to2, grid_dist / lsf, ellipsoid)
        (hemisphere2, zone2, east2, north2, psf2, gridconv2) = geo2grid(
            lat2, lon2, zone1, ellipsoid
        )
        lsf = line_sf(zone1, east1, north1, zone2, east2, north2, hemisphere, ellipsoid)
        lsf_diff = abs(lsf_previous - lsf)

    # Compute Grid Bearing for Station 2
    lat2, lon2, psf2, gridconv2 = grid2geo(zone2, east2, north2, hemisphere, ellipsoid)
...
def vincinv_utm(
    zone1, east1, north1, zone2, east2, north2, hemisphere="south", ellipsoid=grs80
):
    """
    ...
    ell_dist, az1to2, az2to1 = vincinv(pt1[0], pt1[1], pt2[0], pt2[1], ellipsoid)
...
def line_sf(
    ...
    r_sq_m = rho(lat_mean, ellipsoid) * nu(lat_mean, ellipsoid) * projection.cmscale**2
    k1 = (eastofcm1**2 + eastofcm1 * eastofcm2 + eastofcm2**2) / (6 * r_sq_m)
    k2 = (eastofcm1**2 + eastofcm1 * eastofcm2 + eastofcm2**2) / (36 * r_sq_m)
...
def rho(lat, ellipsoid=grs80):
    ...
    return (ellipsoid.semimaj * (1 - ellipsoid.ecc1sq)) / (
        1 - ellipsoid.ecc1sq * (sin(radians(lat)) ** 2)
    ) ** 1.5
