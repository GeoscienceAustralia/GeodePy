...
def rect_radius(ellipsoid):
    ...
    return (
        ...
        / (1 + nval)
        * (
            (nval2 * (nval2 * (nval2 * (25 * nval2 + 64) + 256) + 4096) + 16384)
            / 16384.0
        )
    )
...
def alpha_coeff(ellipsoid):
            ...
            * (
                ...
                * (
                    ...
                    * (
                        nval
                        * (
                            nval * ((37884525 - 75900428 * nval) * nval + 42422016)
                            - 89611200
                        )
                        + 46287360
                    )
        ...
        * (
            ...
            * (
                ...
                * (
                    nval
                    * (
                        nval * (nval * (148003883 * nval + 83274912) - 178508970)
                        + 77690880
                    )
                    + 67374720
                )
    ...
    a6 = (
        ...
        * (
            ...
            * (
                nval
                * (
                    nval * (nval * (318729724 * nval - 738126169) + 294981280)
                    + 178924680
                )
                - 234938880
            )
    ...
    a8 = (
        ...
        * (
            nval
            * (
                nval * ((14967552000 - 40176129013 * nval) * nval + 6971354016)
                - 8165836800
            )
            + 2355138720
        )
    ...
    a10 = (
        nval**5
        * (nval * (nval * (10421654396 * nval + 3997835751) - 4266773472) + 1072709352)
    ) / 2490808320.0

    a12 = (
        nval**6 * (nval * (175214326799 * nval - 171950693600) + 38652967262)
    ) / 58118860800.0

    a14 = (nval**7 * (13700311101 - 67039739596 * nval)) / 12454041600.0

    a16 = (1424729850961 * nval**8) / 743921418240.0
...
def geo2grid(lat, lon, zone=0, ellipsoid=grs80, prj=utm):
    ...
    if prj == isg:
        if zone not in (0, 541, 542, 543, 551, 552, 553, 561, 562, 563, 572):
            raise ValueError(
                "Invalid Zone - Choose from 541, 542, 543, 551, 552, 553, 561, 562, 563, 572"
            )
    else:
    ...
    if prj == isg and ellipsoid != ans:
        warnings.warn(
            message="ISG projection should be used with ANS ellipsoid",
    ...
    if zone == 0:
        if prj == isg:
            amgzone = (float(lon) - (prj.initialcm - (4.5 * prj.zonewidth))) / (
                prj.zonewidth * 3
            )
            ...
            zone = int(
                (float(lon) - (prj.initialcm - (1.5 * prj.zonewidth))) / prj.zonewidth
            )
    if prj == isg:
        amgzone = int(str(zone)[:2])
        subzone = int(str(zone)[2])
        cm = float(
            (amgzone - 1) * prj.zonewidth * 3
            + prj.initialcm
            + (subzone - 2) * prj.zonewidth
        )
    else:
...
def grid2geo(zone, east, north, hemisphere="south", ellipsoid=grs80, prj=utm):
    ...
    def ftn(tn, ecc1):
        return (
            t * sqrt(1 + (sigma(tn, ecc1)) ** 2)
            - sigma(tn, ecc1) * sqrt(1 + tn**2)
            - t1
        )
...
def date_to_yyyydoy(date):
    ...
    try:
        return (
            str(date.timetuple().tm_year) + "." + str(date.timetuple().tm_yday).zfill(3)
        )
