...
class TestHeights(unittest.TestCase):
    ...
    def test_DOVPV(self):
        self.assertAlmostEqual(np.asscalar(heights_s3.interp_DOVPV(Lat,Long)[0]),DOVPV,7)
    def test_DOVPM(self):
        self.assertAlmostEqual(np.asscalar(heights_s3.interp_DOVPM(Lat,Long)[0]),DOVPM,7)
    def test_AHD_H(self):
