...
def grid2geoio(fn, fn_out, easting, northing, utmzone, geotypeout):
    ...
    returned = csvdf.apply(
        lambda x: geodepy.convert.grid2geo(x[utmzone], x[easting], x[northing]), axis=1
    )
    # Convert tuple returned to DafaFrame
    resultdf = pd.DataFrame(
        list(returned),
        columns=["Latitude", "Longitude", "Point Scale Factor", "Grid Convergence"],
    )
    if geotypeout == "DMS":
...
def geo2gridio(fn, fn_out, latitude, longitude, geotypein):
    ...
    returned = csvdf.apply(
        lambda x: geodepy.convert.geo2grid(x[latitude], x[longitude]), axis=1
    )
...
def gdatrans7(fn, fn_out, latitude, longitude, ellht, gdageotypein, direction):
    ...
    if gdageotypein not in gdageotypein_options:
        raise ValueError(
            "Invalid gdageotypein. Expected one of: %s" % gdageotypein_options
        )
    ...
    returned = csvdf.apply(
        lambda x: geodepy.convert.llh2xyz(x[latitude], x[longitude], x[ellht]), axis=1
    )
    ...
    transformed = csvdf.apply(
        lambda x: tf.conform7(x.CartesianX, x.CartesianY, x.CartesianZ, trans), axis=1
    )
    ...
    returned = csvdf.apply(
        lambda x: geodepy.convert.xyz2llh(
            x.TransCartesianX, x.TransCartesianY, x.TransCartesianZ
        ),
        axis=1,
    )
    # Converts the results from llh2xyz into a dataframe
    llhresultdf = pd.DataFrame(
        list(returned), columns=[translat, translong, transellht]
    )
