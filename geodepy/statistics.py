...
def vcv_cart2local(vcv_cart, lat, lon):
    ...
    if column_vector:
        vcv_local = np.array([[vcv_local[0, 0]], [vcv_local[1, 1]], [vcv_local[2, 2]]])
    return vcv_local
...
def vcv_local2cart(vcv_local, lat, lon):
    ...
    if column_vector:
        vcv_cart = np.array([[vcv_cart[0, 0]], [vcv_cart[1, 1]], [vcv_cart[2, 2]]])
    return vcv_cart
...
def error_ellipse(vcv):
    ...
    :return: The orientation of the error ellipse
    """
    z = sqrt((vcv[0, 0] - vcv[1, 1]) ** 2 + 4 * vcv[0, 1] ** 2)
    a = sqrt(0.5 * (vcv[0, 0] + vcv[1, 1] + z))
    b = sqrt(0.5 * (vcv[0, 0] + vcv[1, 1] - z))
    orientation = 90 - degrees(0.5 * atan2((2 * vcv[0, 1]), (vcv[0, 0] - vcv[1, 1])))
...
def circ_hz_pu(a, b):
    ...
    :return: Radius of the circularised error
    """
    q0 = 1.960790
    q1 = 0.004071
    q2 = 0.114276
